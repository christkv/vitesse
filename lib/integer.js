...
/**
 * @fileOverview The IntegerNode class represents an Integer node validation.
 */
    ...
    renderingOptions.type = Mark.up(M(function(){/***
      if(object !== undefined) {
        if((!(typeof object == 'number') || !(typeof object == 'number' && object%1 === 0)) && context.failOnFirst) {
          throw new ValidationError('field is not an integer', path, rules[{{ruleIndex}}], object);
        } else if((!(typeof object == 'number') || !(typeof object == 'number' && object%1 === 0))) {       
          errors.push(new ValidationError('field is not an integer', path, rules[{{ruleIndex}}], object));
        } else {
          // Validations
          {{validations}}       
          // Custom validations
...
var generateValidationLanguage = function(self, validations) {
  var validationTemplate = M(function(){/***
    if(({{validation}}) && context.failOnFirst) {
      throw new ValidationError('integer fails validation {{rule}}', path, rules[{{ruleIndex}}], object);
    } else if(({{validation}})) {
      errors.push(new ValidationError('integer fails validation {{rule}}', path, rules[{{ruleIndex}}], object));
    }
  ***/});
