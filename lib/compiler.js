...
var arrayTemplateFunction =`
  var func{{index}} = function(field, object, parent, rule, context, errors) {
  ...
  for(var i = 0; i < keys.length; i++) {
    ...
    if(value.exists) {
      ...
      if(options.isInArray) {
        ...
        // Function call template
        var functionCallTemplate = `
          func{{index}}("{{field}}", {{object}}, "{{parent}}", rules[{{index}}], context, errors);
        `
    ...
    if(value.type === Array && value.of) {
      ...
      if(options.isInArray) {
        ...
        // Function call template
        var functionCallTemplate = `
          func{{index}}("{{field}}", object[i].{{field}}, f("%s.%s[%s]", parent, field, i), rules[{{index}}], context, errors);
        `
