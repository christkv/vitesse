...
Validator.generate = function generate(key, object, path, context, generateDocumentType) {
  ...
  var validationTemplate = M(function(){/***
    var not_validation{{index}} = function(path, object, context) {
      ...
      console.log("----------------------------------------------- errors.length = " + currentErrors.length)
  ...
  ***/});
  ...
  var callTemplate = "not_validation{{index}}({{path}}, {{object}}, context);"
  // Generate the field
