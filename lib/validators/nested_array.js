...
Validator.generate = function(key, object, path, context, generateDocumentType) {
  ...
  // General for loop
  var forloopTemplate = M(function(){/***
    {{validations}}
  ...
  ***/});
  ...
  // innermost for loop
  var innermostForLoop = M(function(){/***
    {{validations}}
  ...
  ***/});
  ...
  var uniqueValidationTemplate = M(function(){/***
    if(!{{array}}.every(testArrays) && context.failOnFirst) {
      throw new ValidationError('array contains duplicate values', {{path}}, rules[{{ruleIndex}}], {{array}});
    } else if(!{{array}}.every(testArrays)) {      
      errors.push(new ValidationError('array contains duplicate values', {{path}}, rules[{{ruleIndex}}], {{array}}));
    }
  ***/});
  ...
  ***/});
  ...
  } else if(object.object.of instanceof NullType) {
    // Generate the key and path correctly
    innerContext.object = ['object', indexPath].join('');
    // Generate string validation
    NullValidator.generate(key, object.object.of, path, innerContext);
  } else if(object.object.of instanceof NumberType) {
    // Generate the key and path correctly
    innerContext.object = ['object', indexPath].join('');
    // Generate string validation
    NumericValidator.generate(key, object.object.of, path, innerContext);
  } else if(object.object.of instanceof BooleanType) {
    // Generate the key and path correctly
    innerContext.object = ['object', indexPath].join('');
    // Generate string validation
    BooleanValidator.generate(key, object.object.of, path, innerContext);
