...
Validator.generate = function(key, object, path, context, generateDocumentType) {
  ...
  ***/});
  ...
  for(var i = 0; i < schemas.length; i++) {
    ...
    } else if(of instanceof RecursiveReferenceType) {
      console.log("----------------------------- array")
      RecursiveReferenceValidator.generate(key, of, path, innerContext);
    } else if(of instanceof EnumType) {
      EnumValidator.generate(key, of, path, innerContext);
