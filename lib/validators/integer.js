...
Validator.generate = function(key, object, path, context, options) {
  ...
  var callTemplate = M(function(){/***
    ...
    if(!(_object === undefined)) {
      if((!(typeof _object == 'number') || !(typeof _object == 'number' && _object%1 === 0)) && {{performTypeCheck}} && context.failOnFirst) {
        throw new ValidationError('field is not a number', _path, rules[{{ruleIndex}}], _object);
      } else if((!(typeof _object == 'number') || !(typeof _object == 'number' && _object%1 === 0)) && {{performTypeCheck}}) {
  ...
  ***/});
  ...
  for(var operator in validations) {
    if(operator === '$gt') {
      ...
      valueValidations.push(f('(_object / %s % 1) != 0', validations[operator]));
    } else {
      throw new Error(f('validation operator %s is not supported by Integer type', operator));
    }
  }
