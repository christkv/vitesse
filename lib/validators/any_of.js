  ...
  var validationTemplate = M(function(){/***
    var any_of_validation{{index}} = function(path, object, context) {
      ...
      if(successfulValidations == 0 && context.failOnFirst) {
        throw new ValidationError('value does not match any of the schema\'s in the anyOf rule', path, rules[{{ruleIndex}}], object);
      } else if(successfulValidations == 0 && !context.failOnFirst) {
        errors.push(new ValidationError('value does not match any of the schema\'s in the anyOf rule', path, rules[{{ruleIndex}}], object));
      }
